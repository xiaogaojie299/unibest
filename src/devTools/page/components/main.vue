  ...
  <view class="devTools">
    ...
    <view class="navbar">
      ...
      >
        <view class="left" @click="back">
          <image class="foldIcon" src="@/devTools/page/static/fold.png" />
        ...
        >
          ...
          <image class="refreshIcon" src="@/devTools/page/static/refresh.png" />
        </view>
      </view>
    </view>
    <!-- #endif -->

    <view ref="mask" class="mask" @click="$devTools.hide()"></view>
    ...
    >
      <view class="head">
        ...
        <view v-if="isDialog" class="close" @click="$devTools.hide()">
        ...
        <template #default="{ item, index }">
          ...
          <template v-if="item.title == 'Pages'">
            ...
            <template v-if="item.isShowData">
              <cell v-for="(_item, _index) in item.data" :key="_item.route">
                <pageItem :item="_item" :key="_index" />
              </cell>
            </template>
            <cell>
              <view class="cellDivisionLine"></view>
            ...
            <template v-if="item.dayOnlineShow">
              <cell v-for="(_item, _index) in item.dayOnlineList" :key="_index">
                <dayOnlineItem
          ...
          <template v-if="item.title == 'UniBus'">
            ...
            <template v-if="item.isShowCount">
              <cell v-for="_item in item.countList" :key="_item.e">
                <view class="eventItem" @longpress.stop="busCountMenu(_item)">
            ...
            <template v-if="item.isShowLog">
              <cell v-for="(_item, _index) in item.logList" :key="_index">
                <view class="eventLogItem" @longpress.stop="busLogMenu(_item)">
          ...
          <template v-if="item.title == 'FileSys'">
            ...
            <cell
              v-if="item.data.length == 0 || (item.data.length == 1 && item.data[0].type == 'back')"
            >
              <view class="devEmpty">
          ...
          <template v-if="item.isLongList">
            <cell v-if="!item.isLoadAll && item.data.length == 30">
              <view class="loadAll" @click="showListAll">
                <image class="loadAllFold" src="@/devTools/page/static/unfold.png" />
            ...
            <cell
              v-if="(item.type == 'loading' || item.type == 'loading') && item.data.length == 0"
            >
              <view class="devEmpty">
    ...
    <addStorage :storageType="tabList[5].storageType" @getPage="getPage" />
...
export default {
  ...
  mounted() {
    ...
    uni.$on('devTools_delError', (item) => {
      ...
      let i = that.tabList[tabIndex].data.findIndex(
        (x) =>
          x.type == item.type && x.t == item.t && x.m == item.m && x.tr == item.tr && x.p == item.p,
      )
      if (i != -1) {
        that.tabList[tabIndex].data.splice(i, 1)
      }
  ...
  methods: {
    ...
    onrefresh() {
      if (
        !this.tabList[this.tabIndex].canRefreshing ||
        this.tabList[this.tabIndex].type == 'loading'
      )
        return false
      this.getPage(true)
    },
    ...
    getConsole() {
      ...
      if (
        data.length > 30 &&
        that.tabList[tabIndex].isLongList &&
        !that.tabList[tabIndex].isLoadAll
      ) {
      ...
      data = data.map((x) => {
        ...
        x.icon =
          x.type == 'dir'
            ...
            : dirReader.getFileIcon(x.fileType)
        return x
      })
